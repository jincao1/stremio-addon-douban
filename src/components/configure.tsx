...
export const Configure: FC<ConfigureProps> = ({
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        const res = await client.index.$post({ json: value }, { headers: { "X-User-ID": userId } });
        const result = await res.json();
        if ("success" in result && result.success && result.manifestUrl) {
          setManifestUrl(result.manifestUrl);
          setSavedConfig(value); // 更新已保存的配置
          toast.success(userId ? "配置已保存" : "配置链接已生成");
          if (!userId) {
            setUserId(result.userId);
            window.history.replaceState(null, "", result.manifestUrl.replace("manifest.json", "configure"));
          }
        } else {
          toast.error("保存失败");
        }
              ...
              >
                <ItemGroup className="rounded-lg border">
                  <form.Field name="imageProviders" mode="array">
                    {(field) => <ImageProviderSortable value={field.state.value} onChange={field.handleChange} />}
                  </form.Field>
                </ItemGroup>
              </SettingSection>
        ...
        <div className="page-container shrink-0 px-4 pt-4">
          <form.Subscribe selector={(state) => [state.values.catalogIds, state.isSubmitting, state.values] as const}>
            {([catalogIds, isSubmitting, values]) => {
              ...
              const saveButtonText = userId ? "保存配置" : "生成配置链接";
              const loadingText = userId ? "保存中..." : "生成中...";
